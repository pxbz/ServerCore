...
public class PrivateChatCommand extends BaseCommand {
    ...
    public CommandTriState executeCommand(@NotNull CommandSender sender, @NotNull String commandLabel, @NotNull String[] args) {
        ...
        ChatManager.togglePrivateChat(target);

        if (target != sender) {
            CoreUtils.sendIfNotBlank(sender, ChatManager.inPrivateChat(target)?
                    Messages.PRIVATE_CHAT_ON_OTHER.replaceAll("\\{TARGET}", target.getName()) :
                    Messages.PRIVATE_CHAT_OFF_OTHER.replaceAll("\\{TARGET}", target.getName()));
        }

        CoreUtils.sendIfNotBlank(target, ChatManager.inPrivateChat(target)?
                Messages.PRIVATE_CHAT_ON : Messages.PRIVATE_CHAT_OFF);
